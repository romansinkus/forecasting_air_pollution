# lachlan_models/train_cnn_lstm.py
...
class CNNLSTM_MultiHead(nn.Module):
    def __init__(self, in_channels, seq_len, num_reg_targets=5, num_classes=3,
                 conv_channels=32, lstm_hidden=64, lstm_layers=1):
        super().__init__()
        ...
        # Heads
        self.fc_shared = nn.Linear(lstm_hidden, 128)
...
def run_epoch(model, loader, reg_criterion, cls_criterion, optimizer=None,
              device="cpu", lambda_reg=1.0, lambda_cls=1.0):

    train = optimizer is not None
    model.train() if train else model.eval()

    total_reg_loss, total_cls_loss, total_samples, correct = 0, 0, 0, 0
...
def main():
    ...
    parser.add_argument("--save_path", type=str, default="cnn_lstm_multitask.pt")
    args = parser.parse_args()

    OUTPUT_DIR = os.path.join(os.path.dirname(__file__), "outputs", "cnn_lstm")
